...
public interface SettleOrderMapper extends BaseMapper<SettleOrder> {
    ...
    @Select("select batch_no FROM settle_order where institution_code = #{institutionCode} and bank_code=#{bankCode} and settle_type=1 and date(create_time)=curdate() LIMIT 1")
