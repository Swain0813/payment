 ...
 * @description:
 * @author: YangXu
 * @create: 2019-06-27 16:00
 **/
@Component
@Slf4j
public class WechatMQReceive {
    @Autowired
     ...
     * @param value
     */
    @RabbitListener(queues = "MQ_TK_WECHAT_QQSB_DL")
    public void processTKXSQQSB(String value) {
        ...
        if (rabbitMassage.getCount() > 0) {
            ...
            log.info("----------------- MQ_TK_WECHAT_QQSB_DL 退款请求失败对列  ----------------  : {} ", value);
            ...
            if (response.getCode().equals("200")) {//请求成功
                ...
                if (map != null && map.get("return_code") != null && map.get("return_code").equals("SUCCESS")
                    ...
                    if (!cFundChange.getCode().equals(String.valueOf(AsianWalletConstant.HTTP_SUCCESS_STATUS))) {//请求失败
                        ...
                        log.info("----------------- 退款操作 上报队列 MQ_QJS_TZSB_DL -------------- massage : {} ", JSON.toJSON(massage));
            ...
            //预警机制
            messageFeign.sendSimple(developerMobile, "退款请求失败对列(wechat) MQ_TK_WECHAT_QQSB_DL 预警");//短信通知
            messageFeign.sendSimpleMail(developerEmail, "退款请求失败对列(wechat) MQ_TK_WECHAT_QQSB_DL 预警", "MQ_TK_WECHAT_QQSB_DL 预警 ：{ " + value + " }");//邮件通知
        }
    }
}
